...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    len_judge = [count_non_a_elements(strn) for strn in texts]
    boxes_judge = instance.gt_boxes
    box_judge = [[float(box[2]-box[0]), float(box[3]-box[1])] for box in boxes_judge]
    ss_vector = []
    # ss_vector （bs，n_q，n_pts，2）
    # for i in range(len(len_judge)):
    #     if len_judge[i] > 1 and box_judge[i][0] <= box_judge[i][1]:
    #         vector = [[0.0, box_judge[i][1]] for _ in range(25)]
    #         ss_vector.append(vector)
    #     else:
    #         vector = [[box_judge[i][1], 0.0] for _ in range(25)]
    #         ss_vector.append(vector)
    # instance.ss_vector = torch.as_tensor(ss_vector, dtype=torch.float32)
    
    # ss_vector （bs，n_q，1，2）
    for i in range(len(len_judge)):
        if len_judge[i] > 1 and box_judge[i][0] <= box_judge[i][1]:
            vector = [[0.0, box_judge[i][1]]]
            ss_vector.append(vector)
        else:
            vector = [[box_judge[i][1], 0.0]]
            ss_vector.append(vector)
    instance.ss_vector = torch.as_tensor(ss_vector, dtype=torch.float32)
    
    return instance

def count_non_a_elements(lst):
    count = 0
    for element in lst:
        if element != 5462:
            count += 1
