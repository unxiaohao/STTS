...
class DETECTION_TRANSFORMER(nn.Module):
    def __init__(self, cfg, backbone):
        ...
        # spatial semantic vector representation
        self.spatial_semantic_vector = nn.Linear(self.num_points * self.d_model, 2)  # (x, 0) or (0, y)
        ...
        self.spatial_semantic_vector.bias.data = torch.ones(2) * bias_value
        ...
        self.spatial_semantic_vector = nn.ModuleList(
            [self.spatial_semantic_vector for _ in range(num_pred)]
        )
    ...
    def forward(self, samples: NestedTensor):
        ...
        for lvl in range(hs.shape[0]):
            ...
            bs_ss, n_q_ss, n_pts_ss, d_model_ss = hs[lvl].size()
            hs_ss = hs[lvl].reshape(bs_ss, n_q_ss, 1, n_pts_ss * d_model_ss)
            outputs_ss_vector = self.spatial_semantic_vector[lvl](hs_ss)  # bs, n_proposal, 1, 2
            ...
            outputs_ss_vectors.append(outputs_ss_vector)
        ...
        outputs_ss_vector = torch.stack(outputs_ss_vectors)

        out = {
            'pred_logits': outputs_class[-1],
            'pred_text_logits': outputs_text[-1],
            'pred_ctrl_points': outputs_coord[-1],
            'pred_ss_vector': outputs_ss_vector[-1],
    ...
    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_text, outputs_coord, outputs_ss_vector, outputs_bd_coord):
        if outputs_bd_coord is not None:
            return [
                {'pred_logits': a, 'pred_text_logits': b, 'pred_ctrl_points': c, 'pred_bd_points': d, 'pred_ss_vector': e}
                for a, b, c, d, e in zip(outputs_class[:-1], outputs_text[:-1], outputs_coord[:-1], outputs_bd_coord[:-1], outputs_ss_vector[:-1])
            ]
        else:
            return [
                {'pred_logits': a, 'pred_text_logits': b, 'pred_ctrl_points': c, 'pred_ss_vector': d}
                for a, b, c, d in zip(outputs_class[:-1], outputs_text[:-1], outputs_coord[:-1], outputs_ss_vector[:-1])
            ]