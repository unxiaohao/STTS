...
class SetCriterion(nn.Module):
    ...
    def loss_ss_vector(self, outputs, targets, indices, num_inst):
        """Compute the L1 regression loss
        """
        assert 'pred_ss_vector' in outputs
        idx = self._get_src_permutation_idx(indices)
        src_ss_vector = outputs['pred_ss_vector'][idx]
        target_ss_vector = torch.cat([t['ss_vector'][i] for t, (_, i) in zip(targets, indices)], dim=0)
        loss_ss_vector = F.l1_loss(src_ss_vector, target_ss_vector, reduction='sum')
        losses = {'loss_ss_vector': loss_ss_vector / num_inst}
        return losses
