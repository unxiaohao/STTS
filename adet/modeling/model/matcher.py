...
class CtrlPointHungarianMatcher(nn.Module):
    def __init__(
        ...
        self.ss_vector_weight = ss_vector_weight
    ...
    def forward(self, outputs, targets):
        with torch.no_grad():
            ...
            # BOUNDARY cost
            out_bd = outputs["pred_bd_points"].flatten(0, 1).flatten(-2)
            tgt_bd = torch.cat([v["bd_points"] for v in targets]).flatten(-2)
            cost_bd = torch.cdist(out_bd, tgt_bd, p=1)

            # ss vector cost
            out_ss_vector = outputs["pred_ss_vector"].flatten(0, 1).flatten(-2)
            tgt_ss_vector = torch.cat([v["ss_vector"] for v in targets]).flatten(-2)
            cost_ss_vector = torch.cdist(out_ss_vector, tgt_ss_vector, p=1)

            C = self.class_weight * cost_class + self.coord_weight * cost_kpts + \
                self.bd_weight * cost_bd + self.ss_vector_weight * cost_ss_vector
...
def build_matcher(cfg):
    ...
    return BezierHungarianMatcher(class_weight=cfg.BEZIER_CLASS_WEIGHT,
           ...
           CtrlPointHungarianMatcher(class_weight=cfg.POINT_CLASS_WEIGHT,
                                     ...
                                     bd_weight=cfg.BOUNDARY_WEIGHT,
                                     ss_vector_weight=cfg.SPATIAL_SEMANTIC_WEIGHT,
                                     focal_alpha=cfg.FOCAL_ALPHA,
                                     focal_gamma=cfg.FOCAL_GAMMA)